...
| `Key` | User pressed a key while this plugin was focused. Recognised keys: `Up`, `Down`, `Left`, `Right`, `Enter`, `Esc`, `Backspace`, `Tab`, `Char(c)`, `Home`, `End`, `PageUp`, `PageDown` |
...
To add capabilities to your dev setup, include them in the JSON object generated by your dev scripts (or manually edit `plugins.json`). See the radio streaming player's manifest for a working example.
