use santui_ipc::protocol::{Area, HostMsg, IpcKey, IpcKeyModifiers, ThemeData, BORDER_ALL};
use serde_json::{json, Value};
use std::io::{BufRead, BufReader, Write};
use std::process::Command;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum Resource {
    Pods,
    Services,
    Deployments,
    Nodes,
    Namespaces,
}

struct App {
    theme: ThemeData,
    area: Area,
    dirty: bool,
    cached_commands: Vec<Value>,
    resource: Resource,
    namespace: String,
    output: Vec<String>,
    status: String,
}

impl Default for App {
    fn default() -> Self {
        Self {
            theme: default_theme(),
            area: Area { w: 80, h: 24 },
            dirty: true,
            cached_commands: Vec::new(),
            resource: Resource::Pods,
            namespace: "default".into(),
            output: Vec::new(),
            status: "Press r to toggle resource, n to set namespace, Enter to refresh".into(),
        }
    }
}

impl App {
    fn handle_key(&mut self, key: IpcKey, modifiers: IpcKeyModifiers) -> bool {
        self.dirty = true;
        match key {
            IpcKey::Esc => false,
            IpcKey::Char('\n') | IpcKey::Char('\r') => {
                self.fetch();
                true
            }
            IpcKey::Char('r') if !modifiers.ctrl => {
                self.resource = match self.resource {
                    Resource::Pods => Resource::Services,
                    Resource::Services => Resource::Deployments,
                    Resource::Deployments => Resource::Nodes,
                    Resource::Nodes => Resource::Namespaces,
                    Resource::Namespaces => Resource::Pods,
                };
                self.fetch();
                true
            }
            IpcKey::Char('n') if !modifiers.ctrl => {
                self.namespace = if self.namespace == "default" {
                    "kube-system".into()
                } else if self.namespace == "kube-system" {
                    "all".into()
                } else {
                    "default".into()
                };
                self.fetch();
                true
            }
            _ => true,
        }
    }

    fn resource_name(&self) -> &'static str {
        match self.resource {
            Resource::Pods => "pods",
            Resource::Services => "services",
            Resource::Deployments => "deployments",
            Resource::Nodes => "nodes",
            Resource::Namespaces => "namespaces",
        }
    }

    fn fetch(&mut self) {
        let res = self.resource_name();
        self.status = format!("Running kubectl get {res}...");
        let mut cmd = Command::new("kubectl");
        cmd.arg("get").arg(res);
        if self.resource != Resource::Nodes && self.resource != Resource::Namespaces {
            if self.namespace == "all" {
                cmd.arg("--all-namespaces");
            } else {
                cmd.arg("-n").arg(&self.namespace);
            }
        }
        match cmd.output() {
            Ok(out) => {
                let raw = String::from_utf8_lossy(&out.stdout).to_string()
                    + &String::from_utf8_lossy(&out.stderr);
                let lines: Vec<String> = raw.lines().map(|l| l.to_string()).collect();
                self.output = if lines.is_empty() {
                    vec![format!("No {res} found")]
                } else {
                    lines
                };
                self.status = format!("{}: {} lines", res, self.output.len());
            }
            Err(e) => {
                self.output = vec![
                    format!("kubectl error: {e}"),
                    "Ensure kubectl is installed and configured.".into(),
                ];
                self.status = "kubectl failed".into();
            }
        }
    }

    fn render(&mut self) -> Vec<Value> {
        if !self.dirty && !self.cached_commands.is_empty() {
            return self.cached_commands.clone();
        }
        let t = &self.theme;
        let w = self.area.w.max(60);
        let h = self.area.h.max(14);
        let mut cmds: Vec<Value> = Vec::new();

        cmds.push(json!({"Rect": {"x": 0, "y": 0, "w": w, "h": h, "bg": t.background}}));
        cmds.push(json!({"Border": {
            "x": 0, "y": 0, "w": w, "h": h, "fg": t.border, "borders": BORDER_ALL,
            "bg": t.background_panel, "title": " K8s Viewer ",
            "title_fg": t.text, "title_dash_fg": t.border, "border_type": null,
        }}));

        cmds.push(json!({"Text": {
            "x": 2, "y": 1,
            "text": format!("Resource: {} (r toggle)  Namespace: {} (n toggle)", self.resource_name(), self.namespace),
            "fg": t.accent, "bg": null, "bold": true, "modifiers": 0,
        }}));

        let box_y = 3;
        let box_w = w.saturating_sub(4);
        let box_h = h.saturating_sub(5).max(4);

        cmds.push(json!({"Border": {
            "x": 2, "y": box_y, "w": box_w, "h": box_h, "fg": t.accent,
            "borders": BORDER_ALL, "bg": t.background,
            "title": format!(" {} ", self.resource_name()),
            "title_fg": t.accent, "title_dash_fg": t.border, "border_type": null,
        }}));

        if self.output.is_empty() {
            cmds.push(json!({"Text": {
                "x": 4, "y": box_y + 1,
                "text": String::from("Press Enter to fetch resources"),
                "fg": t.text_muted, "bg": null, "bold": false, "modifiers": 0,
            }}));
        } else {
            let max_rows = box_h.saturating_sub(2) as usize;
            for (i, line) in self.output.iter().take(max_rows).enumerate() {
                cmds.push(json!({"Text": {
                    "x": 4, "y": box_y + 1 + i as u16,
                    "text": line.clone(),
                    "fg": t.text, "bg": null, "bold": false, "modifiers": 0,
                }}));
            }
        }

        cmds.push(json!({"Text": {
            "x": 2, "y": h.saturating_sub(2),
            "text": self.status.clone(),
            "fg": t.text_muted, "bg": null, "bold": false, "modifiers": 0,
        }}));
        cmds.push(json!({"Text": {
            "x": 2, "y": h.saturating_sub(1),
            "text": String::from("enter refresh \u{b7} r resource \u{b7} n namespace \u{b7} esc"),
            "fg": t.text_muted, "bg": null, "bold": false, "modifiers": 0,
        }}));

        self.cached_commands = cmds.clone();
        self.dirty = false;
        cmds
    }
}

fn default_theme() -> ThemeData {
    ThemeData {
        text: [220; 3],
        text_muted: [140; 3],
        accent: [180; 3],
        highlight: [220; 3],
        logo: [255; 3],
        background: [0; 3],
        background_panel: [20; 3],
        background_overlay: [10; 3],
        border: [150; 3],
        success: [127, 216, 143],
        error: [224, 108, 117],
        inverted_text: [20; 3],
    }
}

fn palette_commands() -> Value {
    json!([
        {"key": "esc", "hint": "close"},
        {"key": "enter", "hint": "refresh"},
        {"key": "r", "hint": "toggle resource"},
        {"key": "n", "hint": "toggle namespace"},
    ])
}

fn respond(app: &mut App, consumed: bool) {
    let Ok(commands_val) = serde_json::to_value(app.render()) else {
        return;
    };
    let json = json!({
        "commands": commands_val, "hints": [], "palette_commands": palette_commands(),
        "request": null, "plugin_message": null, "consumed": consumed,
    });
    let mut out = std::io::stdout().lock();
    let _ = writeln!(out, "{json}");
    let _ = out.flush();
}

fn main() {
    let _ = env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("warn"))
        .format_timestamp(None)
        .format_target(false)
        .try_init();
    let mut app = App::default();
    let mut reader = BufReader::new(std::io::stdin().lock());
    let mut line = String::new();
    loop {
        line.clear();
        if reader.read_line(&mut line).is_err() || line.is_empty() {
            break;
        }
        let trimmed = line.trim();
        if trimmed.is_empty() {
            continue;
        }
        let msg = serde_json::from_str::<HostMsg>(trimmed);
        let consumed = match msg {
            Ok(HostMsg::Init { theme, area, .. }) => {
                app.theme = theme;
                app.area = area;
                app.dirty = true;
                false
            }
            Ok(HostMsg::Resize { area }) => {
                app.area = area;
                app.dirty = true;
                false
            }
            Ok(HostMsg::ThemeChange { theme }) => {
                app.theme = theme;
                app.dirty = true;
                false
            }
            Ok(HostMsg::Key { key, modifiers }) => app.handle_key(key, modifiers),
            Ok(HostMsg::PaletteCommand { .. }) => {
                app.dirty = true;
                true
            }
            Ok(HostMsg::Shutdown) => break,
            Ok(
                HostMsg::Tick
                | HostMsg::Focus
                | HostMsg::Blur
                | HostMsg::UserUpdate { .. }
                | HostMsg::DbValue { .. }
                | HostMsg::PluginMessage { .. }
                | HostMsg::Mouse { .. },
            ) => false,
            Err(e) => {
                log::error!("[k8s-viewer] parse error: {e}: {trimmed}");
                false
            }
        };
        respond(&mut app, consumed);
        line.clear();
    }
}
