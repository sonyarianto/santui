...
impl Transform {
    fn label(self) -> &'static str {
        match self {
            ...
            Self::Sort => "Sort lines",
            Self::Dedup => "Dedupe lines",
            Self::Shuffle => "Shuffle lines",
        }
    }
}
...
impl Default for App {
    fn default() -> Self {
        Self {
            ...
            input: String::new(),
            mode: 0,
            status: "Type text \u{b7} tab transform \u{b7} enter apply \u{b7} c copy".into(),
            history: Vec::new(),
            cursor: 0,
        }
    }
...
impl App {
    fn current(&self) -> Transform {
        TRANSFORMS[self.mode]
    }

    fn apply(&self) -> String {
        let t = self.current();
        match t {
            Transform::Lines => format!("{}", self.input.lines().count()),
            Transform::Words => format!("{}", self.input.split_whitespace().count()),
            Transform::Chars => format!("{}", self.input.chars().count()),
            Transform::Reverse => self.input.chars().rev().collect(),
            Transform::Upper => self.input.to_uppercase(),
            Transform::Lower => self.input.to_lowercase(),
            Transform::Sort => {
    ...
    fn handle_key(&mut self, key: IpcKey, modifiers: IpcKeyModifiers) -> bool {
        ...
        match key {
            IpcKey::Tab => {
                self.mode = (self.mode + 1) % TRANSFORMS.len();
                self.status = format!("Transform: {}", self.current().label());
                true
            }
            ...
            IpcKey::Enter => {
                if !self.input.is_empty() {
                    ...
                    let text = out.clone();
                    self.history
                        .insert(0, (self.current().label().to_string(), out));
...
fn render_ui(app: &App) -> Vec<RenderCmd> {
    ...
    cmds.push(RenderCmd::Text {
        x: 2,
        y: 2,
        text: format!("Transform  {}", app.current().label()),
        fg: Some(t.text),
        bg: None,
        bold: false,
        modifiers: 0,
    ...
    let out = app.current_text();
    for (i, line) in out.lines().take(out_box_h as usize - 1).enumerate() {
        cmds.push(RenderCmd::Text {
            x: 4,
    ...
    let list_h = h.saturating_sub(list_start + 2).max(1);
    ...
    let items: Vec<String> = app
        ...
        .take(visible)
        .map(|(name, out)| {
            let preview: String = out.chars().take(24).collect();
            format!("{:<12} {}", name, preview)
        })
        .collect();
    ...
    cmds.push(RenderCmd::Text {
        ...
        text: "type \u{b7} tab transform \u{b7} enter apply+copy \u{b7} c copy \u{b7} \u{2191}\u{2193} history \u{b7} esc".into(),
        fg: Some(t.text_muted),
        bg: None,
        bold: false,
        modifiers: 0,
...
fn palette_commands() -> Vec<(String, String)> {
    vec![("Utilities".into(), "Open text toolkit".into())]
}
...
fn main() {
    ...
    while reader.read_line(&mut line).unwrap_or(0) > 0 {
        ...
        let consumed = match msg {
            ...
            Err(e) => {
                log::error!("[text-toolkit] parse error: {e}: {trimmed}");
                false
            }
        };
