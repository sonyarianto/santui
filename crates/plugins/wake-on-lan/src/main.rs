...
impl Default for App {
    fn default() -> Self {
        Self {
            ...
            port: 9,
            status: "Enter MAC address and press Enter to send WOL packet".into(),
            history: Vec::new(),
        }
    }
}
...
impl App {
    fn handle_key(&mut self, key: IpcKey, modifiers: IpcKeyModifiers) -> bool {
        ...
        match key {
            ...
            IpcKey::Char(c) if !modifiers.ctrl => {
                if c == '\u{7f}' || c == '\x08' {
                    if self.cursor_pos > 0 {
                        self.mac.remove(self.cursor_pos - 1);
                        self.cursor_pos -= 1;
                    }
                } else {
                    self.mac.insert(self.cursor_pos, c);
                    self.cursor_pos += 1;
                }
                true
            }
            ...
            IpcKey::Right => {
                if self.cursor_pos < self.mac.len() {
                    self.cursor_pos += 1;
                }
                true
            }
            ...
            IpcKey::Delete => {
                if self.cursor_pos < self.mac.len() {
                    self.mac.remove(self.cursor_pos);
                }
                true
            }
            _ => true,
    ...
    fn send_wol(&mut self) {
        ...
        if hex.len() != 12 {
            self.status =
                "Invalid MAC address (expected 6 hex bytes, e.g. aa:bb:cc:dd:ee:ff)".into();
            return;
        }
        let bytes: Vec<u8> = (0..6)
            ...
            .collect();
        if bytes.len() != 6 {
            self.status = "Invalid MAC address format".into();
            return;
        }
        let mut magic = vec![0xffu8; 6];
        for _ in 0..16 {
            magic.extend_from_slice(&bytes);
        }
        match UdpSocket::bind("0.0.0.0:0") {
            Ok(socket) => {
                socket.set_broadcast(true).unwrap_or(());
                let addr = format!("{}:{}", self.broadcast_addr, self.port);
                match socket.send_to(&magic, &addr) {
                    Ok(n) => {
                        let msg = format!("Sent {n} bytes to {addr} for MAC {}", self.mac);
    ...
    fn render(&mut self) -> Vec<Value> {
        ...
        cmds.push(json!({"Border": {
            ...
            "bg": t.background_panel, "title": " Wake-on-LAN ",
            "title_fg": t.text, "title_dash_fg": t.border, "border_type": null,
        }}));

        cmds.push(json!({"Text": {
            "x": 2, "y": 1, "text": "MAC Address:", "fg": t.text_muted, "bg": null,
            "bold": true, "modifiers": 0,
        }}));
        cmds.push(json!({"Text": {
            "x": 2, "y": 2,
            "text": if self.mac.is_empty() { String::from("aa:bb:cc:dd:ee:ff") } else { self.mac.clone() },
        ...
        cmds.push(json!({"Border": {
            ...
            "title": " History ", "title_fg": t.accent,
        ...
        if self.history.is_empty() {
            cmds.push(json!({"Text": {
                "x": 4, "y": box_y + 1,
                "text": String::from("No packets sent yet"),
        ...
        cmds.push(json!({"Text": {
            ...
            "text": String::from("type MAC \u{b7} enter send \u{b7} esc"),
            "fg": t.text_muted, "bg": null, "bold": false, "modifiers": 0,
        }}));
...
fn palette_commands() -> Value {
    json!([["Plugins", "Wake On Lan"]])
}

fn key_hints() -> Value {
    json!([["esc", "close"], ["enter", "send WOL"],])
}
...
fn main() {
    ...
    loop {
        ...
        let consumed = match msg {
            ...
            Err(e) => {
                log::error!("[wake-on-lan] parse error: {e}: {trimmed}");
                false
            }
        };
