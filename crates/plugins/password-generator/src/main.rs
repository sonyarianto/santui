use std::io::{BufRead, BufReader, Write};

use rand::RngExt;
use santui_ipc::protocol::{
    Area, HostMsg, IpcKey, IpcKeyModifiers, RenderCmd, TextStyle, ThemeData, BORDER_ALL,
};

const LOWERCASE: &str = "abcdefghijklmnopqrstuvwxyz";
const UPPERCASE: &str = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
const DIGITS: &str = "0123456789";
const SYMBOLS: &str = "!@#$%^&*()-_=+[]{}|;:,.<>?";

#[derive(Debug, Clone)]
struct Entry {
    password: String,
    length: usize,
    lowercase: bool,
    uppercase: bool,
    digits: bool,
    symbols: bool,
}

struct App {
    theme: ThemeData,
    area: Area,
    dirty: bool,
    cached_commands: Vec<RenderCmd>,
    length: usize,
    lowercase: bool,
    uppercase: bool,
    digits: bool,
    symbols: bool,
    history: Vec<Entry>,
    cursor: usize,
    status: String,
}

impl Default for App {
    fn default() -> Self {
        let mut app = Self {
            theme: default_theme(),
            area: Area { w: 80, h: 24 },
            dirty: true,
            cached_commands: Vec::new(),
            length: 16,
            lowercase: true,
            uppercase: true,
            digits: true,
            symbols: false,
            history: Vec::new(),
            cursor: 0,
            status: "Press g to generate".into(),
        };
        app.generate();
        app.cursor = 0;
        app
    }
}

impl App {
    fn char_set(&self) -> String {
        let mut chars = String::new();
        if self.lowercase {
            chars.push_str(LOWERCASE);
        }
        if self.uppercase {
            chars.push_str(UPPERCASE);
        }
        if self.digits {
            chars.push_str(DIGITS);
        }
        if self.symbols {
            chars.push_str(SYMBOLS);
        }
        if chars.is_empty() {
            chars.push_str(LOWERCASE);
        }
        chars
    }

    fn generate(&mut self) {
        let chars = self.char_set();
        let mut rng = rand::rng();
        let password: String = (0..self.length)
            .map(|_| chars.as_bytes()[rng.random_range(0..chars.len())] as char)
            .collect();
        self.history.insert(
            0,
            Entry {
                password,
                length: self.length,
                lowercase: self.lowercase,
                uppercase: self.uppercase,
                digits: self.digits,
                symbols: self.symbols,
            },
        );
        self.history.truncate(100);
        self.cursor = 0;
    }

    fn current_password(&self) -> Option<&str> {
        self.history.get(self.cursor).map(|e| e.password.as_str())
    }

    fn strength(&self) -> (&'static str, [u8; 3]) {
        let charset_size = self.char_set().len();
        let entropy = charset_size as f64;
        let score = (self.length as f64 * entropy.log2()) as u32;
        if score >= 80 {
            ("Strong", [127, 216, 143])
        } else if score >= 50 {
            ("Medium", [255, 200, 100])
        } else {
            ("Weak", [224, 108, 117])
        }
    }

    fn handle_key(&mut self, key: IpcKey, modifiers: IpcKeyModifiers) -> bool {
        self.dirty = true;
        match key {
            IpcKey::Char('g') if !modifiers.ctrl => {
                self.generate();
                self.status = "Generated new password".into();
                true
            }
            IpcKey::Char('l') if !modifiers.ctrl => {
                self.lowercase = !self.lowercase;
                if self.has_any_option() {
                    self.generate();
                }
                self.status = format!("Lowercase: {}", if self.lowercase { "on" } else { "off" });
                true
            }
            IpcKey::Char('u') if !modifiers.ctrl => {
                self.uppercase = !self.uppercase;
                if self.has_any_option() {
                    self.generate();
                }
                self.status = format!("Uppercase: {}", if self.uppercase { "on" } else { "off" });
                true
            }
            IpcKey::Char('d') if !modifiers.ctrl => {
                self.digits = !self.digits;
                if self.has_any_option() {
                    self.generate();
                }
                self.status = format!("Digits: {}", if self.digits { "on" } else { "off" });
                true
            }
            IpcKey::Char('s') if !modifiers.ctrl => {
                self.symbols = !self.symbols;
                if self.has_any_option() {
                    self.generate();
                }
                self.status = format!("Symbols: {}", if self.symbols { "on" } else { "off" });
                true
            }
            IpcKey::Char('+') | IpcKey::Char('=') => {
                self.length = (self.length + 1).min(128);
                self.generate();
                self.status = format!("Length: {}", self.length);
                true
            }
            IpcKey::Char('-') => {
                self.length = (self.length.saturating_sub(1)).max(4);
                self.generate();
                self.status = format!("Length: {}", self.length);
                true
            }
            IpcKey::Char('c') if !modifiers.ctrl => {
                self.copy_current();
                true
            }
            IpcKey::Enter => {
                self.copy_current();
                true
            }
            IpcKey::Up | IpcKey::Char('k') => {
                if self.cursor < self.history.len().saturating_sub(1) {
                    self.cursor += 1;
                }
                true
            }
            IpcKey::Down | IpcKey::Char('j') => {
                self.cursor = self.cursor.saturating_sub(1);
                true
            }
            IpcKey::Esc => false,
            _ => false,
        }
    }

    fn has_any_option(&self) -> bool {
        self.lowercase || self.uppercase || self.digits || self.symbols
    }

    fn copy_current(&mut self) {
        let Some(password) = self.current_password() else {
            return;
        };
        match copy_to_clipboard(password) {
            Ok(()) => self.status = "Copied to clipboard".into(),
            Err(e) => self.status = format!("Clipboard error: {e}"),
        }
    }

    fn render(&mut self) -> &[RenderCmd] {
        if self.dirty || self.cached_commands.is_empty() {
            self.cached_commands = render_ui(self);
            self.dirty = false;
        }
        &self.cached_commands
    }
}

fn render_ui(app: &App) -> Vec<RenderCmd> {
    let mut cmds = Vec::new();
    let t = &app.theme;
    let w = app.area.w.max(42);
    let h = app.area.h.max(14);

    cmds.push(RenderCmd::Rect {
        x: 0,
        y: 0,
        w,
        h,
        bg: t.background,
    });
    cmds.push(RenderCmd::Border {
        x: 0,
        y: 0,
        w,
        h,
        fg: t.border,
        borders: BORDER_ALL,
        bg: Some(t.background_panel),
        title: Some(" Password Generator ".into()),
        title_fg: Some(t.text),
        title_dash_fg: Some(t.border),
        border_type: None,
    });

    let len_str = format!("Length: {} (+/-)", app.length);
    cmds.push(RenderCmd::Text {
        x: 2,
        y: 2,
        text: len_str,
        fg: Some(t.text),
        bg: None,
        bold: false,
        modifiers: 0,
    });

    let option = |enabled: bool, key: &str, label: &str| -> String {
        if enabled {
            format!("[{key}] {label}")
        } else {
            format!(" {key}  {label}")
        }
    };

    let opts = format!(
        "{}  {}  {}  {}",
        option(app.lowercase, "l", "a-z"),
        option(app.uppercase, "u", "A-Z"),
        option(app.digits, "d", "0-9"),
        option(app.symbols, "s", "!@#"),
    );
    cmds.push(RenderCmd::Text {
        x: 2,
        y: 3,
        text: opts,
        fg: Some(t.text_muted),
        bg: None,
        bold: false,
        modifiers: 0,
    });

    let out_box_y = 5;
    let out_box_w = w.saturating_sub(4);
    let out_box_h = 3;
    cmds.push(RenderCmd::Border {
        x: 2,
        y: out_box_y,
        w: out_box_w,
        h: out_box_h,
        fg: t.accent,
        borders: BORDER_ALL,
        bg: Some(t.background),
        title: None,
        title_fg: None,
        title_dash_fg: None,
        border_type: None,
    });

    if let Some(pw) = app.current_password() {
        let display = if pw.len() as u16 <= out_box_w.saturating_sub(4) {
            pw.to_string()
        } else {
            let max = (out_box_w.saturating_sub(5)) as usize;
            format!("{}{}", &pw[..max], "\u{2026}")
        };
        let text_x = 2 + (out_box_w.saturating_sub(display.len() as u16)) / 2;
        cmds.push(RenderCmd::Text {
            x: text_x,
            y: out_box_y + 1,
            text: display,
            fg: Some(t.accent),
            bg: None,
            bold: true,
            modifiers: 0,
        });
    }

    let (strength_label, strength_color) = app.strength();
    let info_y = out_box_y + out_box_h + 1;
    cmds.push(RenderCmd::Text {
        x: 2,
        y: info_y,
        text: format!("Strength: {strength_label}"),
        fg: Some(strength_color),
        bg: None,
        bold: true,
        modifiers: 0,
    });

    let hist_y = info_y + 1;
    cmds.push(RenderCmd::Text {
        x: 2,
        y: hist_y,
        text: "History".into(),
        fg: Some(t.text),
        bg: None,
        bold: true,
        modifiers: 0,
    });

    let list_start = hist_y + 1;
    let bottom_space = 3;
    let list_h = h.saturating_sub(list_start + bottom_space).max(1);
    let list_w = w.saturating_sub(4);
    let visible = list_h as usize;
    let total = app.history.len();
    let start = if total <= visible {
        0
    } else {
        app.cursor
            .saturating_sub(visible / 2)
            .min(total.saturating_sub(visible))
    };

    let items: Vec<String> = app
        .history
        .iter()
        .skip(start)
        .take(visible)
        .map(|e| {
            let mut flags = String::new();
            if e.lowercase {
                flags.push('a');
            }
            if e.uppercase {
                flags.push('A');
            }
            if e.digits {
                flags.push('1');
            }
            if e.symbols {
                flags.push('#');
            }
            format!("{:>3} {:6}  {}", e.length, flags, e.password)
        })
        .collect();

    let vis_sel = if app.cursor >= start && app.cursor < start + visible {
        Some(app.cursor - start)
    } else {
        None
    };

    cmds.push(RenderCmd::List {
        x: 2,
        y: list_start,
        w: list_w,
        h: list_h,
        items,
        selected: vis_sel,
        style: TextStyle {
            fg: Some(t.text_muted),
            bg: None,
            bold: false,
            modifiers: 0,
        },
        highlight_style: TextStyle {
            fg: Some(t.inverted_text),
            bg: Some(t.highlight),
            bold: true,
            modifiers: 0,
        },
    });

    cmds.push(RenderCmd::Text {
        x: 2,
        y: h.saturating_sub(2),
        text: app.status.clone(),
        fg: Some(t.text_muted),
        bg: None,
        bold: false,
        modifiers: 0,
    });
    cmds.push(RenderCmd::Text {
        x: 2,
        y: h.saturating_sub(1),
        text: "g generate \u{b7} l a-z \u{b7} u A-Z \u{b7} d 0-9 \u{b7} s symbols \u{b7} +/- length \u{b7} c/\u{23ce} copy \u{b7} \u{2191}\u{2193} history \u{b7} esc".into(),
        fg: Some(t.text_muted),
        bg: None,
        bold: false,
        modifiers: 0,
    });

    cmds
}

fn copy_to_clipboard(text: &str) -> Result<(), String> {
    let mut clipboard = arboard::Clipboard::new().map_err(|e| e.to_string())?;
    clipboard
        .set_text(text.to_owned())
        .map_err(|e| e.to_string())
}

fn default_theme() -> ThemeData {
    ThemeData {
        text: [220; 3],
        text_muted: [140; 3],
        accent: [180; 3],
        highlight: [220; 3],
        logo: [255; 3],
        background: [0; 3],
        background_panel: [20; 3],
        background_overlay: [10; 3],
        border: [150; 3],
        success: [127, 216, 143],
        error: [224, 108, 117],
        inverted_text: [20; 3],
    }
}

fn palette_commands() -> Vec<(String, String)> {
    vec![("Utilities".into(), "Open password generator".into())]
}

fn respond(app: &mut App, consumed: bool) {
    let Ok(commands_val) = serde_json::to_value(app.render()) else {
        return;
    };
    let json = serde_json::json!({
        "commands": commands_val,
        "hints": [],
        "palette_commands": palette_commands(),
        "request": null,
        "plugin_message": null,
        "consumed": consumed,
    });
    if let Ok(json_str) = serde_json::to_string(&json) {
        let mut out = std::io::stdout().lock();
        let _ = writeln!(out, "{json_str}");
        let _ = out.flush();
    }
}

fn main() {
    let _ = env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("warn"))
        .format_timestamp(None)
        .format_target(false)
        .try_init();
    let mut app = App::default();
    let mut reader = BufReader::new(std::io::stdin().lock());
    let mut line = String::new();
    while reader.read_line(&mut line).unwrap_or(0) > 0 {
        let trimmed = line.trim_end();
        let msg = serde_json::from_str::<HostMsg>(trimmed);
        let consumed = match msg {
            Ok(HostMsg::Init { theme, area, .. }) => {
                app.theme = theme;
                app.area = area;
                app.dirty = true;
                false
            }
            Ok(HostMsg::Resize { area }) => {
                app.area = area;
                app.dirty = true;
                false
            }
            Ok(HostMsg::ThemeChange { theme }) => {
                app.theme = theme;
                app.dirty = true;
                false
            }
            Ok(HostMsg::Key { key, modifiers }) => app.handle_key(key, modifiers),
            Ok(HostMsg::PaletteCommand { .. }) => {
                app.generate();
                app.dirty = true;
                true
            }
            Ok(HostMsg::Shutdown) => break,
            Ok(
                HostMsg::Tick
                | HostMsg::Focus
                | HostMsg::Blur
                | HostMsg::UserUpdate { .. }
                | HostMsg::DbValue { .. }
                | HostMsg::PluginMessage { .. }
                | HostMsg::Mouse { .. },
            ) => false,
            Err(e) => {
                log::error!("[password-generator] parse error: {e}: {trimmed}");
                false
            }
        };
        respond(&mut app, consumed);
        line.clear();
    }
}
