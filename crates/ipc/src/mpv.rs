//! Minimal libmpv wrapper loaded at runtime via `libloading`.
//!
//! Shared by the radio, quran, music-preview and iptv plugins. `Mpv::new`
//! takes the plugin's audio client name plus any plugin-specific options.
...
// Values match libmpv's `mpv_event_id` enum.
...
impl Mpv {
    /// Load libmpv and create a player instance.
    ///
    /// `app_name` is passed as the `audio-client-name` option; `extra_options`
    /// are additional `(option, value)` pairs applied before initialization.
    /// Returns the player plus any non-fatal option/init warnings.
    pub fn new(
        app_name: &str,
    ...
    pub fn toggle_pause(&self) -> Result<(), Box<dyn std::error::Error>> {
        self.command(&["cycle", "pause"])
    }

    pub fn set_pause(&self, pause: bool) -> Result<(), Box<dyn std::error::Error>> {
        self.set_property("pause", if pause { "yes" } else { "no" })
    }
    ...
    /// Radio-style title lookup: stream-title first, falling back to media-title.
