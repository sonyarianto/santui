...
- **plugins-manifest.json**: This checked-in file (`plugins-manifest.json`) is the **source of truth** for all first-party plugin metadata (id, name, description, capabilities). Both dev mode (`scripts/dev-setup.sh`) and CI (`release.yml`) read this file to generate platform manifests. When adding a new plugin, you MUST add an entry here — otherwise it won't appear in the registry (dev or prod). `plugins.json` (gitignored) is auto-generated and should never be edited by hand.
