...
for bin in "$OUTDIR"/santui-*.exe "$OUTDIR"/santui-*; do
    ...
    PLUGINS+=("$(cat << JSON
{"id":"$id","name":"Radio Streaming Player","description":"Listen to thousands of radio stations worldwide","version":"0.2.1","download_url":"target/debug/$name","sha256":"$hash","size":$size}
JSON
)")
done
