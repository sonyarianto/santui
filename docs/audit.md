...
- [x] `Plugin` trait doesn't require `Send` or `Sync`, preventing future parallel execution — `trait Plugin: Send`; all `Box<dyn Plugin>` → `Box<dyn Plugin + Send>`; `IpcPluginHost` and `MockPlugin` are already Send
